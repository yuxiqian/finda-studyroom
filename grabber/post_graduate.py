    ...
    for camp in range(1, 9):
        for school in school_id:
            ...
            for item in query_result.xpath('//*[@id="table_5"]/tbody/tr'):
                ...
                part = {}
                part['id'] = parse_leaf(item, 'td[1]/div/a')

                part['school'] = parse_leaf(item, 'td[7]/div')
                part['name'] = parse_leaf(
                    item, 'td[2]/div/a').split(part['id'])[0]
                ...
                part['odd'] = []
                part['even'] = []
                part['note'] = remove_nbsp("授课语言：" + language +
                                           "。" + parse_leaf(item, 'td[12]/div'))
                part['ppl'] = int(parse_leaf(item, 'td[10]/div'))
                ...
                for arr in arrs:
                    ...
                    if int(arr_info[0]) + int(arr_info[1]) > 10:
                        ...
                        if term == 3:
                            start_week = int(arr_info[0]) + 19
                            end_week = int(arr_info[1]) + 19
                    ...
                    arr = {
                        'since': start_week,
                        'until': end_week,
                        'weekday': han_numbers.index(arr_info[2]),
                        'begin': int(arr_info[3]),
                        'end': int(arr_info[4]),
                        'room': classroom
                    }
                    ...
                    if odd_even_flag != 1:
                        part['even'].append(arr)

                if part['odd'] == [] and part['even'] == []:
